...
+ Till now, it just support path config file optimize outside the specific directory with `define` style, 
  just execute `gulp example-path` to see the effect. You will get `dist/optimize-outside.js`.